...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.h_projection = nn.Linear(2*hidden_size, hidden_size, bias=None)
        self.c_projection = nn.Linear(2*hidden_size, hidden_size, bias=None)
        self.att_projection = nn.Linear(2*hidden_size, hidden_size, bias=None)
        self.combined_output_projection = nn.Linear(3*hidden_size, hidden_size, bias=None)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for y_t in torch.split(y, 1): # y_t : (1, b, e)
            #print("y_t's shape: {}".format(y.shape))
            squeezed_y_t = y_t.squeeze()
            #print("squeezed_y_t's shape: {}, o_prev's shape: {}".format(squeezed_y_t.shape, o_prev.shape))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #print("dec_state.shape: {}".format(dec_hidden.shape))
        #print("self.att_projection(enc_hiddens): {}".format(enc_hiddens_proj.shape)) # (b, src_len, h)
        unsqueezed_dec_hidden = dec_hidden.unsqueeze(-1)
        ...
        alpha_t = self.softmax(e_t)             # alpha_t.shape = (b, src_len)
        alpha_t = alpha_t.unsqueeze(dim=1)      # alpha_t.shape = (b, 1, src_len)
        a_t = torch.bmm(alpha_t, enc_hiddens)   # a_t = (b, 1, 2h)
        a_t = a_t.squeeze(dim=1)                # a_t = (b, 2h)
